        ...
        with tf.Graph().as_default():
                ...
                if train_model:
                        ...
                        for i in range(len(data['X'])):
                                image = np.expand_dims(data['X'][i],axis = 0)
                                hog = np.expand_dims(data['X2'][i],axis = 0)
                                x = model.predict_label([image,hog])
                                # print (x)
                                yPred.append(x[0][0])
                                x = [k for k,v in enumerate(data['Y'][i]) if v ==1]
                                y.append(x)
                                # print(x[0])
                        ...
                        for i in range(len(validation['X'])):
                                image = np.expand_dims(validation['X'][i],axis = 0)
                                hog = np.expand_dims(validation['X2'][i],axis = 0)
                                x = model.predict_label([image,hog])
                                # print (x)
                                yPred.append(x[0][0])
                                x = [k for k,v in enumerate(validation['Y'][i]) if v ==1]
                                y.append(x)
                                # print(x[0])
                        ...
                        print( "Test samples: {}".format(len(validation['Y'])))
                        print( "--")
                        print( "evaluating...")
                        start_time = time.time()
                        validation_accuracy = evaluate(model, validation['X'], validation['X2'], validation['Y'])
