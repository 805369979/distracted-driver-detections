...
def create_model():
    ...
    model.add(Convolution2D(32, 3,3, input_shape=img.shape))
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(2, 2)))

    # 第二层CNN
    model.add(Convolution2D(64, 6, 6, input_shape=img.shape))
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    # # # # # # #
    # # # # # 第三层CNN
    model.add(Convolution2D(128, 9, 9, input_shape=img.shape))
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    # # # # # #
    # # # # # # # 第四层CNN
    model.add(Convolution2D(256, 12, 12, input_shape=img.shape))
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
